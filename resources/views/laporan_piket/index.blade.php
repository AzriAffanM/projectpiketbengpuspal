<x-app-layout>
    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto px-6">
            <h1 class="text-2xl font-semibold text-emerald-900 mb-4">Laporan Piket</h1>
            ...
            <div class="bg-white/95 shadow-xl ring-1 ring-emerald-100 rounded-xl overflow-hidden">
                <table class="min-w-full divide-y divide-emerald-100">
                    ...
                    <tbody class="divide-y divide-emerald-100">
                                ...
                                <td class="px-4 py-2 text-sm">
                                    @php
                                        $statusClass = match($laporan->status) {
                                            ...
                                            default => 'bg-yellow-50 text-yellow-800 ring-yellow-200',
                                        };
                                    @endphp
                                    <span class="inline-flex items-center px-2 py-1 text-xs font-medium rounded-full ring-1 ring-inset {{ $statusClass }}">
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-4 py-6 text-center text-gray-500 text-sm">Belum ada laporan piket.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            @if (auth()->check() && auth()->user()->role === 'admin')
                <div class="mt-4 flex justify-end">
                    <a href="{{ route('laporan-piket.admin-index') }}" class="inline-flex items-center px-4 py-2 rounded-lg border border-emerald-300 text-emerald-800 hover:bg-emerald-50 text-sm">Buka Laporan Piket (Admin)</a>
                </div>
            @endif
